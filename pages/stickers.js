        ...
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            ...
            {Object.keys(products).map((item) => {
              return <div key={products[item]._id} className="lg:w-1/4 md:w-1/2 p-4 w-full shadow-lg cursor-pointer" onClick={() => router.push(`/product/${products[item].slug}`)}>
                <a className="block relative  rounded overflow-hidden">
                  {/*eslint-disable-next-line*/}
                  <img alt="ecommerce" className=" md:h-[36vh] h-[30vh] block  m-auto" src={products[item].img} />
                </a>
                <div className="mt-4 text-center md:text-left">
                  <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">{products[item].category}</h3>
                  <h2 className="text-gray-900 title-font text-lg font-medium">{products[item].title}</h2>
                  <p className="mt-1"><b>Rs.</b> {products[item].price}</p>
                  <div className='mt-2'>
                  ...
                  <div className='mt-2'>
                    ...
                    {products[item].color.includes("white") &&
                      <button className="border-2 border-gray-300 ml-1 bg-white-700 rounded-full w-6 h-6 focus:outline-none"></button>}
...
export async function getServerSideProps() {
  ...
  let products = await Product.find({ category: 'stickers' || 'sticker' })
  let stickers = {}
